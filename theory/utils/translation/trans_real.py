...
class TheoryTranslation(gettext_module.GNUTranslations):
  """
  This class sets up the GNUTranslations context with regard to output
  charset. Theory uses a defined DEFAULT_CHARSET as the output charset on
  Python 2.4.
  """
  def __init__(self, *args, **kw):
    ...
    try:
      ...
      pass
    self.theory_output_charset = 'utf-8'
    self.__language = '??'
...
def translation(language):
  ...
  def _fetch(lang, fallback=None):
    ...
    def _translation(path):
      try:
        t = gettext_module.translation('theory', path, [loc], TheoryTranslation)
        t.set_language(lang)
        return t
      except IOError:
        return None
...
def check_for_language(lang_code):
  ...
  for path in all_locale_paths():
    if gettext_module.find('theory', path, [to_locale(lang_code)]) is not None:
      return True
  return False
...
def get_language_from_request(request):
  ...
  if hasattr(request, 'session'):
    lang_code = request.session.get('theory_language', None)
  ...
  for accept_lang, unused in parse_accept_lang_header(accept):
    ...
    for lang, dirname in ((accept_lang, normalized),
      ...
      if lang.lower() not in supported:
        continue
      for path in all_locale_paths():
        if os.path.exists(os.path.join(path, dirname, 'LC_MESSAGES', 'theory.mo')):
...
# get_date_formats and get_partial_date_formats aren't used anymore by Theory
...
def get_date_formats():
  ...
  """
  warnings.warn(
    "'theory.utils.translation.get_date_formats' is deprecated. "
...
def get_partial_date_formats():
  ...
  """
  warnings.warn(
    "'theory.utils.translation.get_partial_date_formats' is deprecated. "
    "Please update your code to use the new i18n aware formatting.",
    DeprecationWarning
  )
  from theory.conf import settings
