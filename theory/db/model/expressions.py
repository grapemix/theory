...
class ExpressionNode(tree.Node):
  ...
  def containsAggregate(self, existingAggregates):
    if self.children:
      return any(child.containsAggregate(existingAggregates)
            for child in self.children
            if hasattr(child, 'containsAggregate'))
    else:
      return refsAggregate(self.name.split(LOOKUP_SEP),
                 existingAggregates)
  ...
  def prepare(self, evaluator, query, allowJoins):
    return evaluator.prepareNode(self, query, allowJoins)

  def evaluate(self, evaluator, qn, connection):
    return evaluator.evaluateNode(self, qn, connection)
...
class F(ExpressionNode):
  ...
  def prepare(self, evaluator, query, allowJoins):
    return evaluator.prepareLeaf(self, query, allowJoins)

  def evaluate(self, evaluator, qn, connection):
    return evaluator.evaluateLeaf(self, qn, connection)


class DateModifierNode(ExpressionNode):
  """
  Node that implements the following syntax:
  filter(endDate__gt=F('startDate') + datetime.timedelta(days=3, seconds=200))

  which translates into:
  POSTGRES:
    WHERE endDate > (startDate + INTERVAL '3 days 200 seconds')

  MYSQL:
    WHERE endDate > (startDate + INTERVAL '3 0:0:200:0' DAY_MICROSECOND)

  ORACLE:
    WHERE endDate > (startDate + INTERVAL '3 00:03:20.000000' DAY(1) TO SECOND(6))

  SQLITE:
    WHERE endDate > theoryFormatDtdelta(startDate, "+" "3", "200", "0")
  ...
  def evaluate(self, evaluator, qn, connection):
    return evaluator.evaluateDateModifierNode(self, qn, connection)
