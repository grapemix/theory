...
def getVersion(connection):
  ...
  PQServerVersion()/``serverVersion`` connection attribute (available in
  ...
  PQServerVersion()/``serverVersion`` doesn't execute a query so try that
  first, then fallback to a ``SELECT version()`` query.
  """
  if hasattr(connection, 'serverVersion'):
    return connection.serverVersion
  else:
    with connection.cursor() as cursor:
      cursor.execute("SELECT version()")
      return _parseVersion(cursor.fetchone()[0])
