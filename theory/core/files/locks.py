...
if os.name == 'nt':
  import msvcrt
  from ctypes import (sizeof, cUlong, cVoidP, cInt64,
  ...
  if sizeof(cUlong) != sizeof(cVoidP):
    ULONG_PTR = cInt64
  else:
    ULONG_PTR = cUlong
  PVOID = cVoidP
  ...
  class OVERLAPPED(Structure):
    _anonymous_ = ['_offsetUnion']
    _fields_ = [
      ('Internal', ULONG_PTR),
      ('InternalHigh', ULONG_PTR),
      ('_offsetUnion', _OFFSET_UNION),
      ('hEvent', HANDLE)]
  ...
  def lock(f, flags):
    hfile = msvcrt.getOsfhandle(_fd(f))
